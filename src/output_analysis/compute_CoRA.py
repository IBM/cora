...
def main():
    ...
    for idx, row in tqdm(input_data.iterrows(), total=len(input_data)):
        ...
        for evaluation_type in evaluation_data:
            ...
            if to_filter is None or evaluation_type in to_filter:
                ...
                for evaluation_element in curr_evaluation_data:
                    if totals_correct[eval_idx] == -1:
                        totals_correct[eval_idx] = 0
                    eval_idx += 1
        ...
        if corrects == eval_idx:
            total_bmca += 1

        count += 1
    
    MCQAplus = np.mean([ tc/count for tc in totals_correct if tc > 0 ])
    BMCA = total_bmca/count

    CoRA = MCQAplus * (1.0 - (MCQAplus - BMCA))
    print(f"CoRA Accuracy:")
    print(f"   {np.mean(CoRA):.3F}")
